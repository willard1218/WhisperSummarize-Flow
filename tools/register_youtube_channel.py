...
def main() -> int:
    ...
    parser.add_argument(
        "--recipient-group",
        help="Local recipient group name stored in config/recipient_groups.local.json",
    )
    parser.add_argument(
        "--config",
        default="config/youtube_subscriptions.json",
        help="Path to the YouTube subscriptions JSON file (default: config/youtube_subscriptions.json)",
    )
